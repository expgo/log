...
var _NameNameMap = map[string]Name{
	"no":    NameNo,
	"short": NameShort,
	"full":  NameFull,
}
...
func (x Name) Name() string {
	if v, ok := _NameNameMap[string(x)]; ok {
		return string(v)
	}
	return fmt.Sprintf("Name(%s).Name", string(x))
}
...
func ParseName(value string) (Name, error) {
	if x, ok := _NameNameMap[value]; ok {
		return x, nil
	}
	if x, ok := _NameNameMap[strings.ToLower(value)]; ok {
		return x, nil
	}
	return "", fmt.Errorf("%s is %w", value, ErrInvalidName)
}
